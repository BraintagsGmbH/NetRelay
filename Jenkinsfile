   ...
   stage 'Build'
    ...
    wrap([$class: 'ConfigFileBuildWrapper', managedFiles: [[fileId: 'MAVEN_SETTINGS_GLOBAL_OSSHR', 
    	replaceTokens: false, targetLocation: 'settings.xml' ]]]) {
         sh "mvn -s settings.xml -Dsign.skip=true -DNetRelayPort=9898 -DstartMongoLocal=false -DlocalPort=27018  -Dconnection_string=mongodb://192.168.42.180:27017 clean deploy"
