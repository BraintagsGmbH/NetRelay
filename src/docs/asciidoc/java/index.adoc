...
`link:../../apidocs/de/braintags/netrelay/controller/impl/FailureController.html[FailureController]`, where resulting errorpages can be defined in
...
NetRelay contains already several ready to use controllers, which can be simply
...
extending `link:../../apidocs/de/braintags/netrelay/controller/impl/AbstractController.html[AbstractController]`, for instance. +
...
and the snippet from the html template looks like that:
